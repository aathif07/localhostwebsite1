...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex w-full flex-1 items-center justify-between py-6 text-left font-medium transition-all hover:bg-white/5 [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn("pb-6 pt-0 text-gray-400 leading-[25.6px]", className)}>
      {children}
    </div>
  </AccordionPrimitive.Content>
))
